...
BEGIN
  ...
  SELECT
    ...
    r.return_message::text AS last_return_message,
    avg7.avg_ms AS avg_duration_ms_7d,
  ...
  LEFT JOIN LATERAL (
    SELECT AVG(EXTRACT(EPOCH FROM (end_time - start_time)) * 1000) AS avg_ms
    FROM cron.job_run_details
    WHERE jobid = j.jobid
