...
import { DemoWalkthrough, useLaunchTour } from "@/components/DemoWalkthrough";
      ...
      {users.map((u) => (
        <div key={u.user_id} className="p-3 rounded-sm bg-surface-elevated border border-border">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className="text-[9px] font-mono text-muted-foreground tabular-nums shrink-0">{new Date(u.created_at).toLocaleDateString()}</span>
          </div>
              ...
              return (
                <button
                  ...
                  className={`inline-flex items-center gap-1 text-[10px] font-mono px-2 py-1 rounded-sm border transition-all ${
                    has ? ROLE_COLORS[role] : "border-border/50 text-muted-foreground/50 hover:border-border hover:text-muted-foreground"
                  }`}
                >
                  {loading ? <Loader2 className="h-2.5 w-2.5 animate-spin" /> : has ? <Minus className="h-2.5 w-2.5" /> : <Plus className="h-2.5 w-2.5" />}
                  {ROLE_LABELS[role]}
                </button>
              );
            })}
        ...
        <div key={r.id} className="p-3 rounded-sm bg-surface-elevated border border-border">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button onClick={() => handleDelete(r.id)} disabled={deleting === r.id} className="p-1 rounded-sm text-muted-foreground hover:text-crisis-red hover:bg-crisis-red/10 transition-colors">
...
export default function SettingsPage() {
  ...
  const { data: trackingRuleSummary = { total: 0, active: 0 } } = useQuery({
    queryKey: ["tracking-rule-summary"],
    enabled: isAdmin,
    queryFn: async () => {
      const { data, error } = await supabase.from("tracking_rules").select("id, is_active");
      if (error) throw error;
      return {
        total: data.length,
  ...
  const addKeyword = () => {
    ...
    setSettings((prev) => ({ ...prev, keywords: [...prev.keywords, keyword] }));
    setNewKeyword("");
  };

  const removeKeyword = (keyword: string) => {
    setSettings((prev) => ({ ...prev, keywords: prev.keywords.filter((item) => item !== keyword) }));
  };
  ...
  return (
    <AppLayout>
      <div className="max-w-7xl space-y-6">
        ...
        {isAdmin && (
          <Card>
            <CardHeader>
              ...
              <p className="text-xs text-muted-foreground mt-1">Monitoring keywords have moved to a dedicated admin page for faster access.</p>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-3">
                <StatPill>{trackingRuleSummary.total} total rules</StatPill>
                <StatPill>{trackingRuleSummary.active} active now</StatPill>
                <StatPill>1-click access from sidebar</StatPill>
              </div>
              <div className="flex flex-col gap-3 rounded-sm border border-border bg-surface-elevated p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm font-medium text-foreground">Open Keyword & Queries Manager</p>
                  <p className="text-xs text-muted-foreground">Add tracked names, map them to cases, and manage ingestion rules there.</p>
                </div>
                <Button asChild className="font-mono text-xs uppercase tracking-wider">
                  <Link to="/tracking-manager">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                ...
                <Input value={settings.spikeMultiplier} onChange={(e) => setSettings((prev) => ({ ...prev, spikeMultiplier: e.target.value }))} className="mt-1 font-mono text-sm bg-card" />
              ...
              <div>
                ...
                <Input value={settings.influencerThreshold} onChange={(e) => setSettings((prev) => ({ ...prev, influencerThreshold: e.target.value }))} className="mt-1 font-mono text-sm bg-card" />
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm font-mono uppercase tracking-wider">Personal Workspace Keywords</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="mb-3 text-xs text-muted-foreground">These keywords are only for your personal workspace preferences. They do not control monitoring ingestion.</p>
            <div className="flex flex-wrap gap-1.5 mb-3">
              {settings.keywords.map((keyword) => (
                <Badge key={keyword} variant="secondary" className="font-mono text-xs cursor-pointer hover:bg-destructive/20 transition-colors" onClick={() => removeKeyword(keyword)}>
                  {keyword} <X className="h-3 w-3 ml-1" />
                </Badge>
              ))}
            </div>
            <div className="flex gap-2">
              <Input placeholder="Add keyword..." className="text-sm font-mono bg-card" value={newKeyword} onChange={(e) => setNewKeyword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addKeyword()} />
              <Button variant="outline" size="sm" className="font-mono text-xs whitespace-nowrap" onClick={addKeyword}>Add</Button>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            {notificationItems.map((item) => (
              <div key={item.key} className="flex items-center justify-between gap-3">
                ...
                <Switch checked={settings.notifications[item.key]} onCheckedChange={(value) => updateNotification(item.key, value)} />
              </div>
            ))}
          </CardContent>
        </Card>
